...
function RightSidebar({selectedUser}) {
  return selectedUser && (
    <div className={`bg-[#8185852]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? "max-md:hidden":""}`}>
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {imagesDummyData.map((url, index)=>(
            ...
            className='cursor-pointer rounded'>
                 <img src={url} alt='loading' className='h-full rounded-md'/>
                 {console.log(url)}
            </div>
          ))}
        </div>
